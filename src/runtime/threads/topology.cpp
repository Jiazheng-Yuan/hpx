...
namespace hpx { namespace threads
{
    ...
    HPX_NATIVE_TLS hpx_hwloc_bitmap_wrapper topology::bitmap_storage_(nullptr);

    threads::mask_type topology::get_area_membind_nodeset(
        const void *addr, std::size_t len) const
    {
        hpx_hwloc_bitmap_wrapper& nodeset = topology::bitmap_storage_;
        if (!nodeset)
        {
            topology::bitmap_storage_.reset(hwloc_bitmap_alloc());
        }
        //
        hwloc_membind_policy_t policy;
